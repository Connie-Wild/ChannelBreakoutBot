...
|core|number/null|optimizationで使用するCPUコア数を指定。`null`の場合、全てのコアを利用する。`1`の場合、同期処理となりパラメータ毎の実行結果を表示するが、`2`以上または`null`の場合は、非同期処理となるためパラメータ毎の実行結果は表示されず、最後の最適値のみ表示される。(全てのコアを利用するとCPU使用率が100%に張り付くため、全体コア数-1の値を設定する事をオススメする。)|
